            ...
            if(err!==null){
                ...
                collection.find().toArray(function(err,collection){
                    if(err!==null){
                        ...
                        for(var i=0;i<collection.length;i++){
                            userDynamicStr += '{"poiid":"'+collection[i]._id+'","count":"'+collection[i].value.count+'","date":"'+collection[i].value.dates+'"},';
                            if(i===collection.length-1){
...
//多天数据提取生成json文件
exports.workuserInformation = function(req,res) { 
    var dat=req.body.date;
    var name=req.body.name;
    var userStr="";
    function getdate(dat){
    ...
    return new promise(function(resolve, reject) {
        userDataModel.checkData.collection.mapReduce(map,reduce,{
        query:{ "date":{"$in":dat}},out:name,keeptemp:true
        },function(err,collection){
            if(err!==null){
                res.status(400).send('Error');
                console.log('I will not easily go die ,but I have err------'+err);
