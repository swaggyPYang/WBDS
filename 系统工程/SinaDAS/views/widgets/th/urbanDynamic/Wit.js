    ...
    function(declare,lang,parser,BaseWidget,on,query,dom,domConstruct,WidgetManager,topic,HorizontalSlider,
        ...
        return declare([BaseWidget], {
            ...
            startup: function() {
                ...
                this.ageTimeSlider = new HorizontalSlider({
                             ...
                             onChange: function(value){      
                                ...
                                case 6:
                                self.dycleanMap();
                                self.displayResult(GpAgeResultLayer6);
                                      break;
                                case 7:
                                self.dycleanMap();
                                self.displayResult(GpAgeResultLayer7);
                                      break;
                                case 8:
                                self.dycleanMap();
                                self.displayResult(GpAgeResultLayer8);
                                      break;      
                                default:
                                }                               
                             }
            ...
            startDoubleTime:function(){
                ...
                on(dojo.byId('compaerLayer'),'click',function(){
                    var layerone =GpGridResultLayer1.value.features ;
                    map.addLayers([layerone]);  
                    var layertwo =GpGridResultLayer2.value.features ;
                    map.addLayers([layertwo]);  
                    // 创建卷帘 
                    var father = dom.byId('main');
                    domConstruct.create("div",{ id: "swipeDiv"}, father, "first");
                            
                    self.layerSwipe = new LayerSwipe({
                        type: "vertical",
                        top: 250,
                        map: map,
                        layers: [layertwo]
            ...
            startGenderAna :function(){
                ...
                var topic_startGender = topic.subscribe("some/topic_startGender_isDone", function(e){
                    if(e[0]==="startGenderLayer" && e[1]==='1'){
                        ...
                        dom.byId('stateBack').innerHTML = jobstatus;
                        dom.byId('resultLayer2').innerHTML = "男性图层B成成功";
                        }
                });
            ...
            startGenderGp:function(sexID,sexLayer,sexStartDate,sexStartTime,sexEndTime,sexType){
                var self = this;
                var sexID = sexID;
                var sexLayer = sexLayer;
                var sexStartDate = sexStartDate;
                var sexStartTime = sexStartTime;
                var sexEndTime = sexEndTime;
                var sexType = sexType
                ...
                dom.byId('stateBack').innerHTML = jobstatus;
                var genderdeferredResult = dojo.xhrPost({
                ...
                genderdeferredResult.then(function(response){
                    console.log(response);
            ...
            startAgeTime:function(){
                ...
                // 清除图层
                on(dojo.byId('cleanageTimeLayer'),'click',function(){
                    dojo.unsubscribe(topic_startAllTime);
